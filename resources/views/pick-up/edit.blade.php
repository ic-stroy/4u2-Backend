...
@section('content')
            ...
            <form action="{{route('pick_up.update', $address->id)}}" class="parsley-examples" method="POST">
                ...
                <div class="row size_store mb-3">
                    ...
                    <div class="col-6">
                        ...
                        <input type="number" name="postcode" class="form-control" value="{{$address->postcode??''}}">
                    </div>
    ...
    <script>
        let page = true
        @if($address && isset($address->cities))
            let current_region = "{{$address->cities->region?$address->cities->region->id:''}}"
